...
from PySide6.QtWidgets import QApplication, QMainWindow, QHBoxLayout, QVBoxLayout, QWidget, QColorDialog
...
#Ensure only one ground node
class Ground_Error(Exception):
    pass
...
class NodeItem(QGraphicsEllipseItem):
    ...
    def __init__(self, x, y,qcolor):
        self.x = x
        self.y = y
          # Naughty! Should not embed magic constants
        self.nodeFillBrush = QBrush(Qt.black, Qt.SolidPattern)
        self.nodeFillBrush.setColor(qcolor)
        boundingRect = QRectF(x - self.nodeRadius, y - self.nodeRadius, 2.0 * self.nodeRadius,
                              2.0 * self.nodeRadius)  # Bounding rectangle of node 'ellipse' i.e. circle
    ...
    def paint(self, painter, option, parent):
        ...
        painter.drawText(boundingRect, Qt.AlignCenter,
                         self.nodeText)  # Clips nodeText. TODO - generate more accurate bounding retangle for text
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.openMenuAction = self.fileMenu.addAction("&Quit")
        self.openMenuAction.triggered.connect(self.on_quit_action)
        ...
        self.graphicsScene = QGraphicsScene(0, 0, 511, 511,
                                            self)  # Naughty! Should not be embedding 'magic' constants in a program!!!
        ...
        nodeA = NodeItem(250, 250,nodeFillColour)
    ...
    def Ground_Node_action(self):
        if(self.count_GN==0):
                ...
                Ground_Node = NodeItem(250, 250,qcolor)
                self.graphicsScene.addItem(Ground_Node)
                Ground_Node.setNodeText("Ground node")
                self.count_GN=1
        else:
            raise Ground_Error("Only one ground node is allowed")
    ...
    def Internal_Node_action(self):
        ...
        qcolor = QColorDialog.getColor()
        print(qcolor.name(), "\n")
        Internal_Node = NodeItem(250, 250,qcolor)
    ...
    def Current_Source_action(self):
        ...
        qcolor = QColorDialog.getColor()
        print(qcolor.name(), "\n")
        Current_Source = NodeItem(250, 250,qcolor)
...
if __name__ == "__main__":
    ...
    exit(
        application.exec_())  # Not sure why this still has to be `exec_` with a trailing underscore? Despite what the documentation says...