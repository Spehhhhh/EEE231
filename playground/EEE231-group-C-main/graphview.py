...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.zoom_in = self.mainToolBar.addAction("Zoom In")
        self.zoom_in.triggered.connect(self.on_zin_action)
        self.zoom_out = self.mainToolBar.addAction("Zoom Out")
        self.zoom_out.triggered.connect(self.on_zout_action)
    ...
    def on_zin_action(self):
        # Handler zoom in
        self.zoom(1.1)

    def on_zout_action(self):
        # Handler zoom out
        self.zoom(0.9)

    def zoom(self, magn: float):
        # magn cant be 0, no point
        if not magn:
            raise ValueError("magn cant be {}".format(magn))
        self.graphicsView.scale(magn, magn)
