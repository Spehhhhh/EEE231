class GraphController:
    node_counter = {"GroundNode": 0, "SourceNode":0}  #dictionary for the number of ground and source nodes
    arc_counter = {} #dictionary that uses the node IDs as keys and the number of arcs it is connected to as value pair
    a=0
    b=0
    ...
    def insert_component (self, graph_instance, parameters):
        ...
        if parameters ["type"] == "Arc":
            ...
            if parameters["node1"] == "SourceNode" :
                if parameters["node1_uid"] in self.arc_counter:               #if it exist in the arc_counter, there is already 1 arc connected to source node
            ...
            if (valid1 == 1) and (valid2 == 1) and (valid3 == 1):
                ...
                self.add_arc (parameters["node1_uid"],parameters["node2_uid"]) #pass the 2 node IDs to be used as keys in the arc_counter dictionary
                return temp      
