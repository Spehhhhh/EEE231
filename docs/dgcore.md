...
The class defines all of the components needed for Graph class, itself has a lot of attritubes like name, uid, color and parent graph. Among which, uid is a string of randome characters which can be used to check which component does the uid belong to. The uid was generated by the method `generate_uid()`. When instantiating a new object, you can also get attributes by using method like `get_parent_graph()`, `get_uid()`, `get_name()`, `get_colour()`.
